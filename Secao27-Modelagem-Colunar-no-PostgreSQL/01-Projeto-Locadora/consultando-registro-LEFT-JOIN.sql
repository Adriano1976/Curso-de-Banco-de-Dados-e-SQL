...
FROM GENERO G
LEFT JOIN FILME F ON G.IDGENERO = F.ID_GENERO
LEFT JOIN LOCACAO L ON F.IDFILME = L.ID_FILME;
...
"TOMATES VERDES FRITOS"	"DRAMA"			
"GUERRA CIVIL"			"AVENTURA"			
*/



-- LEFT JOIN (EXCLUSIVO): Retorna somente os registros da tabela à esquerda que 
-- NÃO tiverem correspondência com registros da tabela à direita.

SELECT F.NOME, G.NOME, L.DATA, L.DIAS, L.MIDIA 
FROM GENERO G
LEFT JOIN FILME F ON G.IDGENERO = F.ID_GENERO
LEFT JOIN LOCACAO L ON F.IDFILME = L.ID_FILME
WHERE F.ID_GENERO IS NULL OR L.ID_FILME IS NULL;
