-- RIGHT JOIN (INCLUSIVO): Retorna todos os registros da tabela à direita junto 
-- dos registros correspondentes da tabela à esquerda.

SELECT F.NOME, G.NOME, L.DATA, L.DIAS, L.MIDIA 
FROM GENERO G
RIGHT JOIN FILME F ON G.IDGENERO = F.ID_GENERO
RIGHT JOIN LOCACAO L ON F.IDFILME = L.ID_FILME;
...
-- RIGHT JOIN (EXCLUSIVO): Retorna somente os registros da tabela à direita que 
-- NÃO tiverem correspondência com registros da tabela à esquerda.

SELECT F.NOME, G.NOME, L.DATA, L.DIAS, L.MIDIA 
FROM GENERO G
RIGHT JOIN FILME F ON G.IDGENERO = F.ID_GENERO
RIGHT JOIN LOCACAO L ON F.IDFILME = L.ID_FILME
WHERE G.IDGENERO IS NULL OR F.IDFILME IS NULL;
